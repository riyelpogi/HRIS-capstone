...
<div class="w-11/12 ml-5 mt-5 relative flex gap-5 rounded">
    ...
    <div class="w-11/12 m-5 relative  xsmr:flex-col bg-white smr:justify-center xsmr:justify-center smr:flex-col xsmr:gap-5 smr:gap-5 rounded flex">
            ...
            <div class="w-full relative flex justify-center items-center  bg-whitey flex-wrap gap-5">
                ...
                <div class="w-full relative flex flex-wrap gap-5  xsmr:flex-col smr:flex-col xsmr:justify-center smr:justify-center smr:items-center xsmr:items-center">
                    @if (auth()->user()->two_zero_one_file)
                            <div class="w-3/12 xsmr:w-full smr:w-full flex flex-col border rounded justify-center items-center p-2 overflow-x-hidden relative bg-whitey">
                               <h1 class="text-sm  font-semibold text-center">SSS</h1>
                               <h1 class="cursor-pointer hover:text-red-400 text-center overflow-x-hidden text-xs" wire:click="downloadSss('{{ auth()->user()->two_zero_one_file->sss }}')">
                            {{ auth()->user()->two_zero_one_file->sss  }}</h1>
                            </div>

                            <div class="w-3/12 flex xsmr:w-full smr:w-full flex-col justify-center border rounded items-center p-2 overflow-x-hidden relative bg-whitey">
                                <h1 class="text-sm  font-semibold text-center">TIN</h1>
                                <h1 class="cursor-pointer hover:text-red-400 text-center overflow-x-hidden text-xs" wire:click="downloadTin('{{ auth()->user()->two_zero_one_file->tin }}')">
                             ...
                             <div class="w-3/12 flex xsmr:w-full smr:w-full flex-col justify-center border rounded items-center p-2 overflow-x-hidden relative bg-whitey">
                                ...
                                <h1 class="cursor-pointer hover:text-red-400 text-center overflow-x-hidden text-xs" wire:click="downloadPhilhealth('{{ auth()->user()->two_zero_one_file->philhealth }}')">
                               {{ auth()->user()->two_zero_one_file->philhealth  }}</h1>
                             </div>
                             
                             <div class="w-3/12 flex xsmr:w-full smr:w-full flex-col justify-center border rounded overflow-x-hidden bg-red-200 items-center p-2 relative bg-whitey">
                                <h1 class="text-sm  font-semibold text-center">NBI</h1>
                                <h1 class="cursor-pointer hover:text-red-400 text-center text-xs" wire:click="downloadNbi('{{ auth()->user()->two_zero_one_file->nbi }}')">
                               {{ auth()->user()->two_zero_one_file->nbi  }}</h1>
                             </div>

                             <div class="w-3/12 flex xsmr:w-full smr:w-full flex-col justify-center border rounded items-center overflow-x-hidden p-2 relative bg-whitey">
                                <h1 class="text-sm  font-semibold text-center">DIPLOMA</h1>
                                <h1 class="cursor-pointer hover:text-red-400 text-center overflow-x-hidden text-xs" wire:click="downloadDiploma('{{ auth()->user()->two_zero_one_file->diploma }}')">
                               {{ auth()->user()->two_zero_one_file->diploma  }}</h1>
                             </div>

                             <div class="w-3/12 flex xsmr:w-full smr:w-full flex-col justify-center border rounded items-center overflow-x-hidden p-2 relative bg-whitey">
                                <h1 class="text-sm  font-semibold text-center">TOR</h1>
                                <h1 class="cursor-pointer hover:text-red-400 text-center overflow-x-hidden text-xs" wire:click="downloadTor('{{ auth()->user()->two_zero_one_file->tor }}')">
                               {{ auth()->user()->two_zero_one_file->tor  }}</h1>
                             </div>

                             <div class="w-3/12 flex xsmr:w-full smr:w-full flex-col justify-center border rounded items-center overflow-x-hidden p-2 relative bg-whitey">
                                <h1 class="text-sm  font-semibold text-center">RESUME</h1>
                                <h1 class="cursor-pointer hover:text-red-400 text-center overflow-x-hidden text-xs" wire:click="downloadResume('{{ auth()->user()->two_zero_one_file->resume  }}')">
                             ...
                             <div class="w-3/12 flex xsmr:w-full smr:w-full flex-col justify-center border rounded items-center p-2 overflow-x-hidden relative bg-whitey">
                                ...
                                <h1 class="cursor-pointer hover:text-red-400 text-center w-full relative overflow-x-hidden text-xs overflow-y-hidden text-wrap" wire:click="downloadOthers('{{ $file[1] }}')" wire:key='others-{{ $key }}'>
                                    {{ $file[1] }}
                                </h1>
                             </div>
                             @endforeach
        ...
        <div class="w-full relative flex bg-whitey p-5  flex-col justify-center items-center">
            <form wire:submit='requirementUpload' enctype="multipart/form-data">
                ...
                <div class="w-full relative flex flex-col mt-5 mb-5">
                    ...
                    <x-label for="file_image" class="text-xs  mdr:hidden lgr:hidden xlr:hidden 2xlr:hidden 3xlr:hidden p-2 rounded-lg w-full cursor-pointer hover:bg-yellow-green border {{$file_image != null ? 'bg-yellow-green' : ''}}">Upload File:</x-label>
                    <input type="file" wire:model='file_image' class="xsmr:hidden smr:hidden" id="file_image" name="file_image">
                    @error('file_image')
                        <span class="text-xs text-red-200">{{ $message }}</span>
                    @enderror
                </div>
