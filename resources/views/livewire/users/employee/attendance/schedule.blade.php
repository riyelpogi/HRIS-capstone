...
@endif
    ...
    <div class="w-full relative mt-5 flex justify-center items-center">
        ...
        <div class="mr-4 flex gap-5">
            ...
            <x-button wire:click="requestCoe">REQUEST COE</x-button>
    ...
    @elseif($attendance_content == 'REQUESTS')
        <div class="w-full relative flex flex-col justif-center">
            <div class="flex gap-5 ml-5">
                ...
                <button class="p-1 text-xs rounded hover:bg-yellow-green {{$request_content == 'COEREQUEST' ? 'bg-yellow-green' : ''}}"  wire:click="showRequestCoe">COE</button>
           ...
           <div class="w-full mt-5 mb-5 relative flex justify-center" wire:poll>
            ...
            <div class="w-full mt-5 mb-5 relative flex justify-center" wire:poll>
               ...
               @elseif($request_content == 'COEREQUEST')
            <div class="w-full mt-5 mb-5 relative flex justify-center" wire:poll>
                <table class="w-full">
                    <caption class="text-sm border font-semibold">COE REQUEST</caption>
                    <tr class="border">
                        <th class="w-40 text-xs ">DATE</th>
                        <th class="w-40 text-xs ">REASON</th>
                        <th class="w-40 text-xs ">STATUS</th>
                        <th class="w-40 text-xs ">FILE</th>
                        <th class="w-40 text-xs ">REASON TO DECLINE</th>
                        <th class="w-40 text-xs ">ACTION</th>
                    </tr>
                    @if($coe_requests)
                        @foreach ($coe_requests as $coe_request)
                        <tr class="{{ $parameter_id == $coe_request->id ? 'border border-2 border-black' : '' }}">
                            <td class="text-xs text-center">{{$coe_request->created_at}}</td>
                            <td class="text-xs text-center uppercase">{{$coe_request->for}}</td>
                            <td class="text-xs text-center xsmr:hidden smr:hidden">{{$coe_request->status}}</td>
                            @if ($coe_request->file != null)
                            <td class="text-xs text-center cursor-pointer xsmr:hidden smr:hidden" wire:click="downloadCoe('{{ $coe_request->file }}') "  wire:key='downloadCoe-{{ $coe_request->id }}'>{{  $coe_request->file}}</td>
                             @else
                            <td class="text-xs text-center cursor-pointer xsmr:hidden smr:hidden">{{  $coe_request->file}}</td>
                            @endif
                            <td class="text-xs text-center uppercase">{{$coe_request->reason_to_decline}}</td>
                            <td class="text-xs text-center">
                                @if ($coe_request->status != null)
                                    <button class="text-xs p-1 rounded-lg bg-gray-200"  >Cancel</button>
                                @else
                                    <button class="text-xs p-1 rounded-lg bg-red-400 hover:bg-red-200 " wire:click="cancelCoeRequest({{$coe_request->id}})" wire:confirm="Are you sure you want to cancel this?" wire:key="cancelCoeRequest-{{$coe_request->id}}" >Cancel</button>    
                                @endif
                            </td>
                        </tr>
                        @endforeach
...
<x-modal wire:model="coerequestmodal">
    <div class="w-full relative flex justify-center items-center">
        <div class="w-9/12 bg-whitey rounded p-5 relative flex items-center justify-center m-5">
                <form wire:submit="submitRequestCoe" method="POST" >
                    @csrf
                <div class="w-full relative mt-5 mb-5 flex flex-col">
                    <h1 class="font-semibold">Request Certificate Of Employment</h1>
                </div>

                <div class="w-full relative mt-5 mb-5 flex flex-col">
                    <x-label for="coe_for" >COE FOR:</x-label>
                    <x-input type="text" name="coe_for" wire:model="coe_for" id="coe_for" />
                    @error('coe_for')
                    <span class="text-xs text-red-200">{{$message}}</span>
                    @enderror
                </div>
