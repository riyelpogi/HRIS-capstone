...
class Admindashboard extends Component
{
   ...
   public function mount()
   {
         ...
         $employees = User::where('role', 1)
                    ->where('deployed', null)->get();

    foreach ($employees as $key => $employee) {
        if($employee->employee_information){
            if($employee->employee_information->deployment_date != null){
                $date = date('Y-m-d', time());
                if($date == $employee->employee_information->deployment_date){
                    $employee->deployed = true;
                    $employee->save();
                }
            }
        }
